...
export default function EditClientForm({ client }: { client: any }) {
  ...
  const [name, setName] = useState(client.buyerBusinessName);
  const [type, setType] = useState(client.buyerRegistrationType);
  const [ntn, setNtn] = useState(client.buyerNTNCNIC || '');
  const [province, setProvince] = useState(client.buyerProvince);
  const [address, setAddress] = useState(client.buyerAddress);
  const [contactNo, setContactNo] = useState(client.contactNo || '');
  const [branchName, setBranchName] = useState(client.branchName || '');
    ...
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      <div className="flex justify-end gap-4">
        <button 
          ...
          className="px-8 py-3 rounded-xl font-bold transition-all text-neutral-400 hover:text-white"
        >
          Cancel
        </button>
        <button 
        ...
        >
          ...
          {isLoading ? 'Saving...' : 'Update Client Profile'}
        </button>
      </div>
    </form>
  );
